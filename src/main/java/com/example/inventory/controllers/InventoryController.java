...
public class InventoryController {
    ...
    @Autowired
    public InventoryController(InventoryService inventoryService, InputValidator inputValidator) {
     ...
     */
    @Operation(summary = "search a Product by name", description = "When name is supplied as queryparameter, The record for the product is found")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved"),
            @ApiResponse(responseCode = "404", description = "Not found - The product was not found")
    })
    @GetMapping("search")
    public ResponseEntity<ProductDto> searchProductByName( @RequestParam(value = "name") @NotBlank String name) {
        return new ResponseEntity<>(inventoryService.searchProductByName(name), HttpStatus.OK);
    }

    @Operation(summary = "Update the quantity", description = "When id and quantity are supplied, The quantity of the product is added to the currently available stock")
    @ApiResponses(value = {
